...
function EventsCategory() {
  ...
  const fetchEventsCategory = async () => {
    setLoading(true);
    axios
      .get(`${baseURL}/photographer/event-category`, { headers: authHeaders() })
      .then((response) => {
        setLoading(false);
      ...
      .catch((error) => {
        ...
        if (statusCode === 403 || errorMessage.includes("trial period")) {
          setPermission(true);
        }
      });
  };
  ...
  const performSearch = () => navigate(`/photographer/search/${searchTerm}`);
  ...
  if (permission) {
    return (
      <div className="bg-slate-100 p-5 rounded text-center mt-5">
        ...
        <h1 className="text-slate-700 font-normal text-2xl">You do not have access to this page</h1>
