...
function Events() {
  ...
  const { videoStatus, stepeventid, setStepEventid, stepsubeventid } =
  ...
  const handleNext = () => {
    if (
      ...
      toast.error("You can’t perform another upload while already uploading.");
      return;
    }
              ...
              {savedStep === steps.length ? (
                ...
                <>
                  ...
                  {savedStep === 0 && (
                    <div className="text-start my-5 w-full">
                      ...
                      <Select
                        ...
                        value={options.find(
                          (opt) =>
                            opt.value === eventId || opt.value === stepeventid,
                        )}
                        onChange={(selectedOption) => {
                          setEventId(selectedOption);
                          setStepEventid(selectedOption);
                          setEventname(selectedOption.label);
                        }}
                        placeholder={
                          status === "loading" || videoStatus === "loading"
                  ...
                  <div className="flex justify-between gap-5">
                    {back === true && status !== "loading" && videoStatus !== "loading" && (
                      <button
