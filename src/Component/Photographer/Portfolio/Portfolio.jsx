...
function Portfolio() {
  ...
  const profileImageSrc = profilePreview || profile;
  const coverImageSrc = coverPreview || cover;
  const [aspectRatio, setAspectRatio] = useState(1);
  const [coverMobilePreview, setCoverMobilePreview] = useState(null);
  const [coverMobileImageLoaded, setCoverMobileImageLoaded] = useState(false);
  const coverMobileImageSrc = coverMobilePreview || cover;
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    setCoverMobileImageLoaded(false);
  }, [coverMobileImageSrc]);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      coverMobile: portfolio?.coverMobile || null,
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        formData.append("coverMobile", values.coverMobile);
        ...
        const res = await axios.post(`${baseURL}/portfolio`, formData, {
          headers: {
  ...
  const handleImageChange = (event, type) => {
    ...
    setCropType(type);
    setAspectRatio(aspectRatios[type]);
    ...
    <>
      <section>
        {dataload ? (
          ...
          <>
            <div className="bg-white p-4 rounded md:mt-2 dark:bg-slate-800 text-start">
              ...
              <form onSubmit={formik.handleSubmit}>
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4 dark:bg-slate-900/30">
                    <div>
                      <div className="flex items-center justify-between">
                        ...
                        <label
                          htmlFor="profile"
                          className="flex items-center gap-1 rounded-full border border-slate-300 bg-white px-3 py-1 text-xs text-slate-700 cursor-pointer hover:bg-slate-100"
                        >
                      ...
                      <div className="mt-3 flex items-center gap-4">
                        <div className="relative h-24 w-24 overflow-hidden rounded-full border border-slate-300 bg-white shadow-sm">
                          {!profileImageLoaded && (
                            <Skeleton
                              variant="circular"
                              width="100%"
                        ...
                        <div className="text-sm text-slate-500">
                          <p>Max: 1 MB</p>
                          <p className="mt-1">Recommended: 800 x 800</p>
                        </div>
                      </div>
                ...
                <div className="border border-slate-300 rounded p-3 mt-5">
                  ...
                  {/* Expandable Content */}
                  <div
                    className={`grid transition-all duration-500 ease-in-out overflow-hidden ${open
                      ...
                      <div className="flex flex-col md:w-1/2 w-full">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            formik.setFieldValue("pricing", value);
                          }}
                          onKeyDown={(e) => {
                            if (
                              e.key === "-" ||
